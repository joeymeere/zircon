...
import { AUTH_DOMAIN, signInMessage } from "../../../config/auth.config";
...
export const SolanaAuth = (options: SolanaAuthOptions): SolanaAuthHandler => {
  ...
  const getSolanaAuth = async (req: any, res: any) => {
    ...
    if (pubkey) {
      ...
      res.status(401).json("No public key specified");
    }
  };
  ...
  const completeSolanaAuth = async (req: any, res: any) => {
    try {
      ...
      res.status(401).json({ token: undefined, message: err.toString() });
    }
  };
