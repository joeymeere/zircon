      ...
      if (response) {
        if (typeof response == solutionAttributes.expectedType) {
          ...
          if (introspect) {
            ...
            setOutput(
              `The response below is incorrect:\n\n${beautify(response)}`
            );
            toast("❌ Whoops!", {
          ...
          <div className="flex gap-2 items-center justify-end">
            ...
            <button
              onClick={async () => {
                if (correct) {
                  if (publicKey) {
                    await addSolution(id, correct, code, executedTime, {
                  ...
                  toast("⏳ Finish The Question", {
                    description: "Please provide a correct solution to submit.",
                  });
                }
              }}
              className={`inline-flex gap-2 items-center justify-center shadow-[0_0_0_3px_#000000_inset] px-3 py-1.5 bg-transparent border ${
                correct
            ...
            <div className="z-10 sticky top-0 bg-[#09090B] border-b-2 border-[#E851EB]/50 py-2 px-3">
              <div className="flex items-center gap-4 justify-start">
                <button
                  ...
                  className={`px-3 py-1.5 inline-flex gap-2 items-center rounded-md text-white font-plex font-semibold font-neutral-300 ${
                    active == "run"
                      ? "bg-[#E851EB]/50"
                      : "hover:bg-[#E851EB]/25"
                  } transition-colors`}
              ...
              {active == "run" ? (
                <>
                  {loading ? (
                    <svg
                      aria-hidden="true"
                      className="absolute top-12 left-1/2 inline w-8 h-8 text-gray-200 animate-spin dark:text-zinc-400 fill-zinc-400 dark:fill-gray-300"
                      viewBox="0 0 100 101"
                      fill="none"
                    ...
                    <>
                      <button
                        onClick={() => setOutput("")}
                        className={`absolute top-2 right-2 shadow-[0_0_0_3px_#000000_inset] px-2 py-1 bg-transparent ${
                          output.length > 0
                            ? "border border-white text-white"
                            : "border border-neutral-500 text-neutral-500"
                        } rounded-lg font-plex font-semibold text-xs transform hover:-translate-y-1 transition duration-400`}
                      >
                        Clear
                      </button>
                      {output.length > 0 ? (
                        <div className="">
                          <pre
                            ...
                            className="flex w-full h-full mb-0 overflow-auto break-words"
                          >
                            <code className="font-martian text-xs text-neutral-300 w-full js overflow-auto scrollbar max-h-1/3 break-words">
                              {output}
                            </code>
                          </pre>
                        </div>
                      ) : (
                        <p className="font-martian text-sm text-neutral-500">
              ...
              {active == "hints" ? (
                <div className="flex-col gap-6 h-full">
                  {hints.map((hint, i) => (
                    <p
                      key={i}
                      className="font-martian text-sm text-neutral-500"
                    >
                      ✅ {hint}
                    </p>
                  ))}
